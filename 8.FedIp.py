...
for round_idx in range(1, N_ROUNDS + 1):
    ...
    for worker_idx, worker in enumerate(workers):
        ...
        pred = worker.predict(features_scaled_test2)
        plt.figure(figsize=(10, 6))
        plt.plot(pred, label='predictions_test1')
        plt.plot(target_test2, label='predictions_test1')
        plt.title(f'Round {round_idx} - Worker {worker_idx}')
        plt.legend()
        plt.grid()
        plt.savefig(f'./FedIp/round_{round_idx}_worker_{worker_idx}.png')
    ...
    # Update workers with the aggregated parameters
