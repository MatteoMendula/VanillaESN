...
from reservoirpy.nodes import IPReservoir, Reservoir
...
def run_experiment(folder, reservoir, readout, run_with_ip, features_train, target_train, features_test1, target_test1, features_test2, target_test2, RC_size, accuracies_test1, f1_scores_test1, accuracies_test2, f1_scores_test2, training_latencies, inference_latencies1, inference_latencies2):
    ...
    extended_states = np.hstack((train_states, features_train))
    
    print("shape of extended_states", extended_states.shape)

    readout = readout.fit(extended_states[warmup:], target_train[warmup:])
    train_time = (time_ns() - start_time)
    training_latencies.append(train_time)

    plot_tsne(folder, extended_states, target_train, f"RC_size{RC_size} TSNE")

    start_time = time_ns()
    test_states1 = reservoir.run(features_test1)
    extended_states1 = np.hstack((test_states1, features_test1))
    y_pred_1 = readout.run(extended_states1)
    ...
    extended_states2 = np.hstack((test_states2, features_test2))
    y_pred_2 = readout.run(extended_states2)
...
mu = 0.3
warmup = 100
leaking_rates = [0.1, 0.3, 0.5, 0.7, 0.9]
epochs = 100
...
for lr in leaking_rates:
    ...
    _folder = f"{folder}/leaking_rate_{lr}"
...
plot_1_series(folder, leaking_rates, training_latencies, "Training Latency", "leaking_rate", "Latency", "Training Latency")
plot_1_series(folder, leaking_rates, inference_latencies1, "Inference Latency Test1", "leaking_rate", "Latency", "Inference Latency")
plot_1_series(folder, leaking_rates, inference_latencies2, "Inference Latency Test2", "leaking_rate", "Latency", "Inference Latency")
plot_1_series(folder, leaking_rates, accuracies_test1, "Accuracy Test1", "leaking_rate", "Accuracy", "Accuracy")
plot_1_series(folder, leaking_rates, accuracies_test2, "Accuracy Test2", "leaking_rate", "Accuracy", "Accuracy")
plot_1_series(folder, leaking_rates, f1_scores_test1, "F1 Test1", "leaking_rate", "F1", "F1")
plot_1_series(folder, leaking_rates, f1_scores_test2, "F1 Test2", "leaking_rate", "F1", "F1")